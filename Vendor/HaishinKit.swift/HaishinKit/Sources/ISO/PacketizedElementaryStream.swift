...
struct PacketizedElementaryStream: PESPacketHeader {
    ...
    init?(_ sampleBuffer: CMSampleBuffer?, timeStamp: CMTime) {
        ...
        let mediaSubType = sampleBuffer.formatDescription?.mediaSubType
        switch mediaSubType {
        case .h264:
            if !sampleBuffer.isNotSync {
                data.append(contentsOf: [0x00, 0x00, 0x00, 0x01, 0x09, 0x10])
                if let parameterSets = sampleBuffer.formatDescription?.parameterSets {
                    for parameterSet in parameterSets {
                        data.append(contentsOf: [0x00, 0x00, 0x00, 0x01])
                        data.append(contentsOf: parameterSet)
                    }
            ...
            if let dataBytes = try? dataBuffer.dataBytes() {
                ...
                let byteStream = stream.toByteStream()
                data.append(byteStream)
                if !sampleBuffer.isNotSync, logger.isEnabledFor(level: .info) {
                    logger.info(
                        "H264 PES payload:",
                        " avccBytes=", dataBytes.count,
                        ", annexBBytes=", byteStream.count,
                        ", totalPesBytes=", data.count
                    )
                }
            }
        case .hevc:
        ...
        case .h264, .hevc:
            packetLength = 0
            if logger.isEnabledFor(level: .info), Int(UInt16.max) <= length {
                logger.info(
                    "Video PES using unbounded packet length:",
                    " mediaSubType=", mediaSubType?.rawValue ?? 0,

...
final class VideoCodec {
    ...
    func append(_ sampleBuffer: CMSampleBuffer) {
        ...
        do {
            ...
            if isInvalidateSession {
                if sampleBuffer.formatDescription?.isCompressed == true {
                    ...
                    if logger.isEnabledFor(level: .info) {
                        logger.info(
                            "VideoCodec created compression session:",
                            " videoSize=", settings.videoSize,
                            ", bitRate=", settings.bitRate,
                            ", profileLevel=", settings.profileLevel,
                            ", keyFrameIntervalSec=", settings.maxKeyFrameIntervalDuration
                        )
                    }
                }
            }
            ...
            if sampleBuffer.formatDescription?.isCompressed == true {
                try session.convert(sampleBuffer, continuation: continuation, forceKeyFrame: false)
            } else {
                if useFrame(sampleBuffer.presentationTimeStamp) {
                    let forceKeyFrame = needsSync
                    try session.convert(sampleBuffer, continuation: continuation, forceKeyFrame: forceKeyFrame)
                    if forceKeyFrame {
                        needsSync = false
                    }
                }

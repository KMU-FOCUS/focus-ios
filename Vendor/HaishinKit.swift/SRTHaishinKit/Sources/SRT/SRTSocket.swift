...
private let kMpegTsPacketSize: Int = 188
private let kMpegTsNullPacket: Data = {
    var packet = Data([0x47, 0x1F, 0xFF, 0x10])
    packet.append(Data(repeating: 0xFF, count: kMpegTsPacketSize - packet.count))
    return packet
}()
...
final actor SRTSocket {
    ...
    func close() {
        ...
        flushPendingOutputBuffer(padRemainder: true)
        srt_close(socket)
    ...
    func send(_ data: Data) {
        ...
        while pendingOutputBuffer.count >= kSRTSOcket_payloadSize {
            let chunk = pendingOutputBuffer.prefix(kSRTSOcket_payloadSize)
            let data = Data(chunk)
            pendingOutputBuffer.removeFirst(kSRTSOcket_payloadSize)
    ...
    private func flushPendingOutputBuffer(padRemainder: Bool) {
        guard !pendingOutputBuffer.isEmpty else {
            return
        }
        if padRemainder {
            let originalBytes = pendingOutputBuffer.count
            let remainder = pendingOutputBuffer.count % kSRTSOcket_payloadSize
            if remainder != 0 {
                let neededBytes = kSRTSOcket_payloadSize - remainder
                let nullPacketCount = Int(ceil(Double(neededBytes) / Double(kMpegTsPacketSize)))
                for _ in 0..<nullPacketCount {
                    pendingOutputBuffer.append(kMpegTsNullPacket)
                }
                if logger.isEnabledFor(level: .info) {
                    logger.info(
                        "SRT payload tail flush:",
                        " originalBytes=", originalBytes,
                        ", paddedBytes=", pendingOutputBuffer.count,
                        ", nullPacketCount=", nullPacketCount
                    )
                }
            }
        }
